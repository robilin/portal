...
class Purchase_Model extends MY_Model
{
  ...
  public function select_product_purchase_history()
    {
        $this->db->select('tbl_purchase_product.*', false);
        $this->db->select('tbl_purchase.*',false);
        $this->db->from('tbl_purchase_product');
        $this->db->join('tbl_purchase', 'tbl_purchase.purchase_id  =  tbl_purchase_product.purchase_id ', 'left');
        $this->db->group_by(array("unit_price","supplier_id"));
        $query_result = $this->db->get();
        $result = $query_result->result();
